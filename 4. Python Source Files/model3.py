...
# 3️⃣ Define model building function for Keras Tuner
def build_model(hp):
    model = Sequential()
    ...
    model.add(Dropout(hp.Float('dropout_1', 0.1, 0.4, step=0.05)))
    ...
    if hp.Boolean('second_layer'):
        ...
        model.add(BatchNormalization())
        model.add(Dropout(hp.Float('dropout_2', 0.05, 0.3, step=0.05)))
    ...
    if hp.Boolean('third_layer'):
        ...
        model.add(BatchNormalization())
        model.add(Dropout(hp.Float('dropout_3', 0.05, 0.3, step=0.05)))
...
# 5️⃣ Get best model and train further if needed
...
# Save the best model
model.save("dnn_pm25_kathmandu_hourly_merra_tuned.keras")
print("✅ Tuned DNN model saved as dnn_pm25_kathmandu_hourly_merra_tuned.keras")
...
print("✅ DNN Hourly Merra PM2.5 Results (After Tuning):")
